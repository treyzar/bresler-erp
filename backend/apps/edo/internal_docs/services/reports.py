...
def stuck_documents(min_pending_days: int = 3) -> list[dict]:
    ...
    docs = (
        Document.objects.filter(status=Document.Status.PENDING, submitted_at__lt=threshold)
        ...
        out.append(
            {
                ...
                "current_approver": (step.approver.get_full_name() if (step and step.approver) else ""),
            }
        )
    return out
...
def sla_breaches(period_days: int = 30) -> list[dict]:
    ...
    steps = (
        ApprovalStep.objects.filter(sla_breached_at__gte=since)
