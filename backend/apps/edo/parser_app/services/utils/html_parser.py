...
from .elements import (
    make_text_element, make_image_element, make_table_element, 
    make_signature_element, make_divider_element, is_likely_signature, generate_id
)
...
def parse_html(file_obj) -> Dict[str, Any]:
    ...
    for tag in soup.find_all(target_tags):
        ...
        if tag.find_parent('table') and tag.name != 'table': continue
        # Пропускаем div-обертки, если в них нет текста напрямую (чтобы не создавать пустых блоков)
        if tag.name == 'div' and not tag.get_text(strip=True) and not tag.find('img'): continue
        ...
        if tag.name in ['p', 'div', 'li'] or tag.name.startswith('h'):
            ...
            elements.append(make_text_element(
                x=LEFT_MARGIN, y=y_offset, width=CONTENT_WIDTH, height=total_h,
                content=text, size=font_size, bold=is_bold, color=color, align=align
            ))
        ...
        # --- КАРТИНКИ / ПОДПИСИ ---
            ...
            w, h = 200, 150
            try:
                if tag.get('width'): w = int(tag['width'].replace('px','').replace('%','')) # % игнорируем пока
                if tag.get('height'): h = int(tag['height'].replace('px','').replace('%',''))
            except: pass
            ...
            if is_explicit or is_likely_signature(w, h):
                elements.append(make_signature_element(
                    x=LEFT_MARGIN, y=y_offset, width=w, height=h,
                    src=src
                ))
            else:
        ...
        # --- ТАБЛИЦЫ ---
            ...
            if rows_data:
                ...
                elements.append(make_table_element(
                    x=LEFT_MARGIN, y=y_offset, width=CONTENT_WIDTH, height=tbl_h,
                    data=rows_data
                ))
                y_offset += tbl_h + 20
