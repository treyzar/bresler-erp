...
class DepartmentAssignmentFormSet(BaseInlineFormSet):
    """Форсет для DepartmentAssignmentInline: автоматически выставляет
    `Assignment.company` равным company родительского Department, чтобы
    админу не приходилось выбирать его руками.
    """

    def _construct_form(self, i, **kwargs):
        form = super()._construct_form(i, **kwargs)
        if self.instance.pk:
            form.instance.company_id = self.instance.company_id
        return form


class DepartmentAssignmentInline(admin.TabularInline):
    """Штатка подразделения: список Assignment'ов, чьё department=это.

    Каждая строка — один сотрудник в этом отделе с должностью + флагом
    «руководитель» + флагом «основное место». `company` подставляется
    автоматически из родительского Department (через formset).
    """

    model = Assignment
    formset = DepartmentAssignmentFormSet
    extra = 0
    fk_name = "department"
    fields = ("user", "position", "is_head", "is_primary", "is_active", "from_date", "to_date")
...
class DepartmentAdmin(TreeAdmin):
    ...
    inlines = [DepartmentAssignmentInline]
